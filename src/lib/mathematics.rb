...
class Mathematics
  ...
  def eucledian_distance point1 = nil, point2 = nil
    ...
    raise Error, "Points must be of the same dimension" unless( point1.length == point2.length )
    ...
    # Calculate for N dimensions    (p-q)**2
    diffs = []
    0.upto( point1.length - 1 ).each do |index|
      i_point1  = point1[ index ]
      i_point2  = point2[ index ]
      result    = ( i_point2 - i_point1 ) ** 2
      diffs    << result
    end

    # Sum all elements and sqrt
    result = Math.sqrt( diffs.sum )
...
#
  ...
  # @brief    dot_product returns as the name suggest the dot product for two vectors in nD
  #           d(u,v) =  (u).x * (v).x + (u).y * (v).y + (u).z * (v).z ....
  def dot_product( vector1, vector2 )

    sum = 0
    0.upto( vector1.length - 1 ).each do |n|
  ...
  # point_n -> array of [x,y,z]
  def gradient point_n, point_n_plus_one

    # in 3D
    #
    # a        = \frac{\partial}{\partial x} f(x,y,z)
    # b        = \frac{\partial}{\partial y} f(x,y,z)
    # c        = \frac{\partial}{\partial z} f(x,y,z)
    #
    # \nabla f = a i_unit + b j_unit + c k_unit
    # \nabla f = \frac{\partial}{\partial x} f(x,y,z) i_unit + \frac{\partial}{\partial y} f(x,y,z) j_unit + \frac{\partial}{\partial z} f(x,y,z) k_unit

    # \frac{\partial}{\partial x} f(x,y,z) = lim_{\delta x -> 0 } \frac{h(x + \delta x, y, z) - h(x,y,z)}{\delta x}
    #
    # overkill possibly because points are so close together ?
    # Lets do linear approximation first

    xn1, yn1, zn1 = *point_n
    xn2, yn2, zn2 = *point_n_plus_one


    vector  = [ xn2 - xn1, yn2 - yn1, zn2 - zn1 ]
    norm    = getNorm( vector[0], vector[1], vector[2] )
    # vector.collect!{ |i| i/norm }
